...
const classes = {
    option: (active: boolean) => cntl`
    cursor-pointer relative rounded-2xl 
    select-none py-4 pl-3 pr-9
    ${active ? "bg-dark text-white" : "text-white"}
    `,
    selected: (selected: boolean) => cntl`
    block truncate
    ${selected ? "font-semibold" : ""}
    `,
    selectedValue: (active: boolean) => cntl`
              ...
              >
                {({ active, selected }) => (
                  <>
                    ...
                    {selected && (
                      <span
                        className={classes.selectedValue(active)}
                      >
